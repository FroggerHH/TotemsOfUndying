...
public class Localizer
{
    ...
    private static void LoadLocalization(Localization __instance, string language)
    {
                         ...
                         SearchOption.AllDirectories).Where(f => fileExtensions.IndexOf(Path.GetExtension(f)) >= 0))
        {
            ...
            } else localizationFiles[key] = file;
        }
        ...
        if (LoadTranslationFromAssembly("English") is not { } englishAssemblyData)
        {
            throw new Exception($"Found no English localizations in mod {plugin.Info.Metadata.Name}. "
                                + $"Expected an embedded resource translations/English.json or translations/English.yml.");
        }
        ...
        if (language != "English")
        {
            if (localizationFiles.TryGetValue(language, out var localizationFile1))
            {
        ...
        if (localizationData is null && localizationFiles.TryGetValue("English", out var localizationFile2))
        {
            localizationData = File.ReadAllText(localizationFile2);
        }
